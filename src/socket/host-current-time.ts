...
export async function hostCurrentTime(socket: Socket) {
  socket.on(SOCKET_EVENTS.EVT.HOST_CURRENT_TIME, async (data) => {
    const { partyId, currentTime, runAt, isPaused, currentRate } = data

    const party = await getActiveParty({ partyId })
    if (!party) return

    const session = party.data()
    const user = session.participants.find((p: any) => p.socketId === socket.id)
    ...
    console.log('MOST RECENT DATA =>', partyData)
